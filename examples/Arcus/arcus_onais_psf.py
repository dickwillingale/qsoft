#!/usr/bin/env python
# Ray tracing of Acrcus SPO modules with on-axis source
...
print("finite source distance m",panlen/1000)
print("detector shift for finite distance",dshift)
# set source to cover aperture
slim=np.array([450,-570,1450,570,0,0])
...
a2j,ide,isu)
xsrt.source(4,di,dsrc,spos,sn,rx,slim,0,nray,0)
...
cm=plt.get_cmap("hot")
plt.imshow(np.transpose(arr),extent=[xmin,xmax,ymin,ymax],origin="lower",cmap=cm)
...
rmsa=(bb.rmsa*xsam/flen)*(180/np.pi)*3600
rmsb=(bb.rmsb*xsam/flen)*(180/np.pi)*3600
print("area cm2",area)
print("HEW arc secs",hew)
print("W90 arc secs",w90)
print("FWHM arc secs",fwhm)
print("RMS major arc secs",rmsa)
print("RMS minor arc secs",rmsb)
#
plt.show()

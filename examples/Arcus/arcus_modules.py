...
gaps= 20.0
# azimuthal sector size
asect=np.pi/4
# azimuthal gap
gapw= 15.0
# module aperture width (around ring)
dang= 85.0
...
for i in range(nring):
    ...
    rring[i]= ring
# Fill sector
    na=np.int((ring*asect-gaps)/dang)+1
    dth=(asect-gaps/ring)/na
    mw= dth*ring-gapw
    ...
    hring[i]= ml
    for k in range(na):
        theta=k*dth-asect/2+gaps/ring/2+mw/ring/2
        imod=imod+1
        im.append(imod)
        ir.append(i)
        ii.append(k)
...
zrms=1.0
# in-plane figure errors rms radians (2.4 arc secs)
brms=(2.4/3600)*np.pi/180
...
np.savetxt("arcus_modules.dat",atab.transpose(),

*+QRT_ELTMXT Trace electrons through SVOM MXT telescope with magnetic diverter
        SUBROUTINE QRT_ELTMXT(NPOLE,DM,PDX,PDY,PDZ,DDX,DDY,DDZ,
     +  EKV,TSIG,XAPER,XDIV,NAPER,APY,APZ,APSIZ,WDIV,DDIV,SDET,
     +  MAXST,XP,YP,ZP,NPATH,IQUAL)
        IMPLICIT NONE
        INTEGER NPOLE,NAPER,MAXST,NPATH,IQUAL
        DOUBLE PRECISION DM(NPOLE),EKV,TSIG,XAPER,XDIV
        DOUBLE PRECISION APY(NAPER),APZ(NAPER),APSIZ,WDIV,DDIV,SDET
...
Cf2py  intent(in) XAPER,XDIV,NAPER,APY,APZ,APSIZ,WDIV,DDIV,SDET,MAXST
...
*NAPER   input     number of square apertures in mirror/diverter
*APY    input      y centre of apertures
*APZ    input      z centre of apertures
*APSIZ  input      aperture size (3.8) cm
*WDIV   input      extra width of diverter apertures (0.2) cm
*DDIV   input      axial depth of diverter cm
*SDET   input      size of detector cm (axial position XDET=0.0)
...
*-Author Dick Willingale 2015-Dec-04
        LOGICAL STEPPING
        DOUBLE PRECISION DTOR,RWALL,RAMIN,RAMAX,BF
        DOUBLE PRECISION EV,XX,YY,ZZ,FX,FY,FZ,SM,RR,RMM
        DOUBLE PRECISION DD,YDIV,ZDIV
        DOUBLE PRECISION DFX,DFY,DFZ,BX,BY,BZ,VDT,ETA,PSI,VETA
        EXTERNAL SYS_DRAND
        DOUBLE PRECISION RAN(2),SYS_DRAND,STEP_MAX,XDET,SS,HAPD
        INTEGER I,K,NARC,IAPER
C
        INCLUDE 'QR_COM'
...
C Set radii of mirror aperture and radius of aft body wall
        ...
        DO I=1,NAPER
                RAMAX=MAX(RAMAX,
     +          SQRT((APY(I)+APSIZ/2)**2+(APZ(I)+APSIZ/2)**2))
        ENDDO
        RWALL=RAMAX
C Set energy in eV
        EV=EKV*1000.0
...
C 1 tesla=1E4 Gauss so if B in Gauss we require factor 3.37
C for electrons of 3.37*SQRT(1837) for protons to get radius in cm
        ETA =3.37*SQRT(EV)
        PSI = 0.01*ETA*2.0*4.0*ATAN(1.D0)
C Choose random position within rear mirror apertures
        IAPER=int(SYS_DRAND()*NAPER)+1
        YY=SYS_DRAND()*APSIZ+APY(IAPER)-APSIZ*0.5
        ZZ=SYS_DRAND()*APSIZ+APZ(IAPER)-APSIZ*0.5
        XX=XAPER
C Set half width of apertures in diverter
        HAPD=(APSIZ+WDIV)*0.5
...
C Crossed diverter input plane
                        DD=(XDIV-XP(NPATH-1))/(XP(NPATH)-XP(NPATH-1))
                        YDIV=YP(NPATH-1)+(YP(NPATH)-YP(NPATH-1))*DD
                        ZDIV=ZP(NPATH-1)+(ZP(NPATH)-ZP(NPATH-1))*DD
                        I=0
                        STEPPING=.FALSE.
                        IQUAL=4
                        DO WHILE((.NOT.STEPPING).AND.I.LT.NAPER)
                              I=I+1
                              IF((YDIV.GT.(APY(I)-HAPD)).AND.
     +                           (YDIV.LT.(APY(I)+HAPD)).AND.
     +                           (ZDIV.GT.(APZ(I)-HAPD)).AND.
     +                           (ZDIV.LT.(APZ(I)+HAPD))) THEN
                                      STEPPING=.TRUE.
                              ENDIF
                         ENDDO
                ELSEIF(XP(NPATH-1).GE.XDIV-DDIV.AND.
     +          XP(NPATH).LT.XDIV-DDIV) THEN
C Crossed diverter output plane
                        DD=(XDIV-DDIV-XP(NPATH-1))/
     +                  (XP(NPATH)-XP(NPATH-1))
                        YDIV=YP(NPATH-1)+(YP(NPATH)-YP(NPATH-1))*DD
                        ZDIV=ZP(NPATH-1)+(ZP(NPATH)-ZP(NPATH-1))*DD
                        I=0
...
C Crossed focal plane
                        ...
                        IF((YP(NPATH).GT.-SDET*0.5).AND.
     +                     (YP(NPATH).LT.SDET*0.5).AND.
     +                     (ZP(NPATH).GT.-SDET*0.5).AND.
     +                     (ZP(NPATH).LT.SDET*0.5)) THEN

*+SRT_SPOARR  Dynamically set up a pair of square Si pores for SPO array 
	SUBROUTINE SRT_SPOARR(DIR,HPOS,CVR,CAX,CAR,PC,IDEF,ISTAT)
	IMPLICIT NONE
...
* Parameters for module array aperture:
* PC(1) inner radius of aperture
* PC(2) outer radius of aperture
* PC(3) focal length
* PC(4) aperture to join plane distance
* PC(5) spare
* PC(6) spare
* PC(7) spare
* PC(8) local x of ray intersection on aperture surface
* PC(9) local y of ray intersection on aperture surface
* PC(10) surface index of full aperture
* Module parameters returned by call to srt_findspo()
* IMOD aperture index
* RMOD radial position of module
* PMOD azimuthal position of module
* TMOD rotation of module wrt radius vector
* WMOD width of module (radial)
* HMOD height of module (azimuthal)
* PL axial length of module
* CMOD Curvature index 
*  0 conical
*  1 Wolter
*  2 curved-plane
*  3 constant
...
*  6 spherical principal surface with Wolter curvature and aperture grid
...
* GMOD grazing angle ratio
* RAP radial pitch of pores
* WALL wall thickness
* RIB azimuthal pitch of pores (rib spacing)
* RIQ surface quality index
* WFR frame width around module
...
C Get axial curvature 
		ICURV=INT(CMOD)
C GMOD is the grazing angle ratio for this module
...
C Radial packing of pores - find centre of pore for ray using hit position
		...
		RXY=(INT(XX/RAP)+0.5)*RAP+(RMOD-WMOD*0.5)
C
...
C DQ is width used to set up degradation towards axial edges of module

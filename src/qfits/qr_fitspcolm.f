*+QR_FITSPCOLM        put complex (R*8 pairs) as table column
        SUBROUTINE QR_FITSPCOLM(NCOL,NEL,COL,NN,CNAME,NU,CUNIT)
        IMPLICIT NONE
        INTEGER NCOL,NEL,NN,NU
        DOUBLE COMPLEX COL(NEL)
        CHARACTER*(NN) CNAME
        CHARACTER*(NU) CUNIT
...
Cf2py  intent(in) nel
*COL      input        column array of complex (R*8 pairs)
Cf2py  intent(in) col
...
C
        IF(IFITS.EQ.0) THEN
                WRITE(*,*) 'QR_FITSPCOLC - fits file not open'
                ISTAT=1
                RETURN
        ENDIF
...
C Check extension type is binary table
        IF(IHTYPE.NE.2) THEN
                WRITE(*,*)
     +         'QR_FITSPCOLC - current extension not binary table'
                ISTAT=1
                RETURN
        ENDIF
C Check dimensions
        ...
        IF(NCOL.GT.NC) THEN
            WRITE(*,*) 'QR_FITSPCOLC - ncol',NCOL,'too large >',NC
            ISTAT=1
            RETURN
        ENDIF
...
C Put array into column
        CALL FTPCLM(IFITS,NCOL,1,1,NEL,COL,ISTAT)
        END

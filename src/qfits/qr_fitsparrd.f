*+QR_FITSPARRD        put double array as primary or image array
        SUBROUTINE QR_FITSPARRD(NDIMS,NELS,NEL,ARR)
        IMPLICIT NONE
        INTEGER NDIMS,NELS(NDIMS),NEL
        DOUBLE PRECISION ARR(NEL)
...
C
        IF(IFITS.EQ.0) THEN
                WRITE(*,*) 'QR_FITSPARRD - fits file not open'
                ISTAT=1
                RETURN
        ENDIF
...
C Put array into fits
        CALL FTPPRD(IFITS,0,1,NEL,ARR,ISTAT)
        END

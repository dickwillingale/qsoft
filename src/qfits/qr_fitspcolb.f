*+QR_FITSPCOLB        put byte array (raw) as table column
        SUBROUTINE QR_FITSPCOLB(NCOL,NEL,COL,NN,CNAME,NU,CUNIT)
        IMPLICIT NONE
...
Cf2py  intent(in) nel
*COL        input    column array (byte array passed from R as integer)
Cf2py  intent(in) col
...
*-Author: Dick Willingale 2014-Dec-26
        ...
        INTEGER NMAX
        PARAMETER (NMAX=1000)
        CHARACTER*1 BCOL(NMAX)
C
        IF(ISTAT.NE.0) RETURN
C
        IF(IFITS.EQ.0) THEN
                WRITE(*,*) 'QR_FITSPCOLB - fits file not open'
                ISTAT=1
                RETURN
        ENDIF
...
C Check extension type is binary table
        IF(IHTYPE.NE.2) THEN
                WRITE(*,*)
     +            'QR_FITSPCOLB - current extension not binary table'
                ISTAT=1
                RETURN
        ENDIF
C Check dimensions
        ...
        IF(NCOL.GT.NC) THEN
            WRITE(*,*) 'QR_FITSPCOLB - ncol',NCOL,'too large >',NC
            ISTAT=1
            RETURN
        ENDIF
...
C Get column info
        ...
        IF(NREP.GT.NMAX) THEN
                WRITE(*,*) 'QR_FITSPCOLB - repeat',NREP,
     +          'too large >',NMAX
                ISTAT=1
                RETURN
        ENDIF
C Put array into column
        DO I=1,NEL,NREP
                DO J=1,NREP
                        BCOL(J)=ACHAR(COL(I+J-1))
                ENDDO
                CALL FTPCLB(IFITS,NCOL,I,1,NREP,BCOL,ISTAT)
        ENDDO
        END

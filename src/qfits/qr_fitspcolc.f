*+QR_FITSPCOLC        put complex (R*4 pairs) as table column
        SUBROUTINE QR_FITSPCOLC(NCOL,NEL,COL,NN,CNAME,NU,CUNIT)
        IMPLICIT NONE
...
Cf2py  intent(in) nel
*COL      input        column array of complex (R*4 pairs)
Cf2py  intent(in) col
...
C Put array into column
        CALL FTPCLC(IFITS,NCOL,1,1,NEL,COL,ISTAT)
        END

*+QR_FITSPCOLD        put double as table column
        SUBROUTINE QR_FITSPCOLD(NCOL,NEL,COL,NN,CNAME,NU,CUNIT)
        IMPLICIT NONE
        INTEGER NCOL,NEL,NN,NU
        DOUBLE PRECISION COL(NEL)
        CHARACTER*(NN) CNAME
        CHARACTER*(NU) CUNIT
...
C Put array into column
        CALL FTPCLD(IFITS,NCOL,1,1,NEL,COL,ISTAT)
        END

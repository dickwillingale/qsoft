*+QR_FITSGCVJ        Get FITS column integer
        SUBROUTINE QR_FITSGCVJ(NULLVAL,ICOL,IROW,NEL,VAL)
        IMPLICIT NONE
        INTEGER ICOL,IROW,NEL,NULLVAL,VAL(NEL)
...
C
        IF(IFITS.EQ.0) THEN
                WRITE(*,*) 'QR_FITSGCVJ error - file closed'
                ISTAT=1
                RETURN
        ENDIF
C
        CALL FTGCVJ(IFITS,ICOL,IROW,1,NEL,NULLVAL,VAL,LVAL,ISTAT)
        END
